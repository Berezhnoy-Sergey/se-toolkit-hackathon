...
class Settings(BaseSettings):
    ...
    nanobot_tasks_backend_url: str = Field(..., alias="NANOBOT_TASKS_BACKEND_URL")
    nanobot_tasks_api_key: str = Field(..., alias="NANOBOT_TASKS_API_KEY")
...
def _resolve_config() -> Config:
    ...
    # MCP servers
    config.tools.mcp_servers["tasks"] = MCPServerConfig(
        command="opentelemetry-instrument",
        args=["python", "-m", "mcp_tasks"],
        env={
            "NANOBOT_TASKS_BACKEND_URL": env.nanobot_tasks_backend_url,
            "NANOBOT_TASKS_API_KEY": env.nanobot_tasks_api_key,
            **_otel_env(env, "mcp-tasks"),
        },
    )
    # Task 3 — uncomment after you add mcp-obs.
